$(document).ready(function () {
	...
	$('.product-info-color__item').click(function () {
		...
		$(this).closest('.product').find(dot).removeClass('hidden-galery').addClass('curent-order').siblings().addClass('hidden-galery').removeClass('curent-order');
	})
	...
	$('[data-fancybox="g1"], [data-fancybox="g2"],[data-fancybox="g3"],[data-fancybox="g4"],[data-fancybox="g5"],[data-fancybox="g6"],[data-fancybox="g7"],[data-fancybox="g8"],[data-fancybox="g9"] ').fancybox({
		loop: true
	});
	//=========================================================галерея
	...
	function formatDate(args) {
		...
		if (Object.prototype.toString.call(options.date) === '[object Date]') {
			date = options.date;
		} else if (new Date(options.date) !== "Invalid Date" && !isNaN(new Date(options.date))) {
			date = new Date(options.date);
		} else {
			console.log('Incorrect date input format!'); return;
		}
	...
	var formattedDate = formatDate({
		date: tomorrow.setDate(tomorrow.getDate() + 2),
		separator: '.',
		format: 'default',
		locale: 'ru-RU'
	});
	...
	if ($('#modal')) {
		...
		$('.submit-btn').click(function (e) {
			...
			if (!(/\S/.test(valInput)) || !(/\S/.test(valText))) {
				$(this).closest('.rev-form').find('input').css('border', '2px solid red');
				$(this).closest('.rev-form').find('textarea').css('border', '2px solid red');
				$(this).closest('.rev-form').find('.modal-w').css('display', 'block');
			} else {
				modal.close();
				setTimeout(function () { Tmodal.open(); }, 800);
